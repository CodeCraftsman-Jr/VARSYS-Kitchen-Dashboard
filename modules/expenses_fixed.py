...
class ExpensesWidget(QWidget):
    def __init__(self, data, parent=None):
        ...
        self.data = data
        self.expenses_df = data['expenses_list'].copy()
        ...
        # Create tabs for different expenses views
        self.current_list_tab = QWidget()
        self.add_item_tab = QWidget()
        self.expenses_history_tab = QWidget()

        # Add tabs to the tab widget
        self.tabs.addTab(self.current_list_tab, "Current Expenses List")
        self.tabs.addTab(self.add_item_tab, "Add Expenses")
        self.tabs.addTab(self.expenses_history_tab, "Expenses History")
        
        # Set up each tab
        self.setup_current_list_tab()
        self.setup_add_item_tab()
        self.setup_expenses_history_tab()

        # Connect tab change event to refresh data
        self.tabs.currentChanged.connect(self.on_tab_changed)

    def load_data(self):
        """Load and refresh expenses data"""
        try:
            # Refresh the expenses dataframe from the main data
            if 'expenses_list' in self.data:
                self.expenses_df = self.data['expenses_list'].copy()
                self.update_expenses_list()
                print(f"✅ Expenses data loaded: {len(self.expenses_df)} items")
            else:
                print("⚠️ No expenses_list data found in data dictionary")
    ...
    def on_tab_changed(self, index):
        ...
        try:
            ...
            print(f"🔄 Expenses tab switched to: {tab_name}")

            # Force reload expenses data from CSV
            self.reload_expenses_data_from_file()

            if tab_name == "Current Expenses List":
                self.update_expenses_list()
            elif tab_name == "Expenses History":
                self.update_expenses_history()

        except Exception as e:
            print(f"Error in shopping tab change handler: {e}")

    def reload_expenses_data_from_file(self):
        """Force reload expenses data from CSV file"""
        try:
            import pandas as pd
            import os

            expenses_file = 'data/expenses_list.csv'
            if os.path.exists(expenses_file):
                self.data['expenses_list'] = pd.read_csv(expenses_file)
                self.expenses_df = self.data['expenses_list'].copy()
                print(f"🔄 Reloaded expenses data: {len(self.data['expenses_list'])} items")
            else:
                # Create empty expenses list with proper columns
                self.data['expenses_list'] = pd.DataFrame(columns=[
                ...
                self.expenses_df = self.data['expenses_list'].copy()
                print("🔄 Expenses file not found, created empty dataframe")

        except Exception as e:
            print(f"Error reloading expenses data from file: {e}")
    ...
    def setup_current_list_tab(self):
        ...
        header = QLabel("Current Expenses List")
        ...
        self.status_combo.currentIndexChanged.connect(self.update_expenses_list)
        
        # Priority filter
        priority_label = QLabel("Priority:")
        self.priority_combo = QComboBox()
        self.priority_combo.addItems(["All", "High", "Medium", "Low"])
        self.priority_combo.currentIndexChanged.connect(self.update_expenses_list)
        
        # Search
        search_label = QLabel("Search:")
        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText("Search items...")
        self.search_input.textChanged.connect(self.update_expenses_list)
        ...
        self.expenses_table.setColumnCount(13)
        self.expenses_table.setHorizontalHeaderLabels([
            "Item", "Inventory Category", "Budget Category", "Quantity", "Unit", "Priority",
        ...
        default_column_widths = {
            0: 180,  # Item
            1: 120,  # Inventory Category
            2: 120,  # Budget Category
            3: 80,   # Quantity
            4: 60,   # Unit
            5: 80,   # Priority
            6: 100,  # Last Price
            ...
            10: 100, # Avg Price
            11: 120, # Date Added
            12: 120  # Date Purchased
        }

        # Apply column resizing with settings persistence
        self.expenses_table_resizer = apply_universal_column_resizing(
            self.expenses_table,
    ...
    def update_expenses_list(self):
        """Update expenses list with performance optimization"""
        try:
            # Get filter values
            ...
            filtered_df = self.expenses_df.copy() if not self.expenses_df.empty else pd.DataFrame()
            ...
            # Update table
            self.expenses_table.setRowCount(len(filtered_df))
            for i, (_, row) in enumerate(filtered_df.iterrows()):
                self.expenses_table.setItem(i, 0, QTableWidgetItem(str(row['item_name']) if pd.notna(row['item_name']) else ""))
                self.expenses_table.setItem(i, 1, QTableWidgetItem(str(row['category']) if pd.notna(row['category']) else ""))
                # Budget category - column 2
                budget_category = str(row['budget_category']) if 'budget_category' in row and pd.notna(row['budget_category']) else "Kitchen Essentials"
                self.expenses_table.setItem(i, 2, QTableWidgetItem(budget_category))
                self.expenses_table.setItem(i, 3, QTableWidgetItem(str(row['quantity']) if pd.notna(row['quantity']) else "0"))
                self.expenses_table.setItem(i, 4, QTableWidgetItem(str(row['unit']) if pd.notna(row['unit']) else ""))
                self.expenses_table.setItem(i, 5, QTableWidgetItem(str(row['priority']) if pd.notna(row['priority']) else "Medium"))
                ...
                if 'last_price' in row and pd.notna(row['last_price']):
                    self.expenses_table.setItem(i, 6, QTableWidgetItem(f"{currency_symbol} {float(row['last_price']):.2f}"))
                elif 'avg_price' in row and pd.notna(row['avg_price']):
                    # Fallback for avg_price
                    self.expenses_table.setItem(i, 6, QTableWidgetItem(f"{currency_symbol} {float(row['avg_price']):.2f}"))
                else:
                    self.expenses_table.setItem(i, 6, QTableWidgetItem("Soon"))

                # Location - column 7 (consolidated from store)
                try:
                    location_value = ""
                    ...
                    self.expenses_table.setItem(i, 7, QTableWidgetItem(location_value))
                except Exception as e:
                    print(f"Error setting location value: {str(e)}")
                    self.expenses_table.setItem(i, 7, QTableWidgetItem(""))

                # Status - column 8
                try:
                    if 'status' in row and pd.notna(row['status']) and str(row['status']).strip() != '':
                        status_value = str(row['status']).strip()
                        self.expenses_table.setItem(i, 8, QTableWidgetItem(status_value))
                    else:
                        self.expenses_table.setItem(i, 8, QTableWidgetItem("Pending"))
                except Exception as e:
                    print(f"Error setting status value: {str(e)}")
                    self.expenses_table.setItem(i, 8, QTableWidgetItem("Pending"))

                # Current Price - column 9
                try:
                    if 'current_price' in row and pd.notna(row['current_price']):
                        current_price = float(row['current_price'])
                        self.expenses_table.setItem(i, 9, QTableWidgetItem(f"{currency_symbol} {current_price:.2f}"))
                    else:
                        self.expenses_table.setItem(i, 9, QTableWidgetItem("Soon"))
                except Exception as e:
                ...
                try:
                    if 'avg_price' in row and pd.notna(row['avg_price']):
                        ...
                        self.expenses_table.setItem(i, 10, QTableWidgetItem(f"{currency_symbol} {avg_price:.2f}"))
                    else:
                        self.expenses_table.setItem(i, 10, QTableWidgetItem("Soon"))
                except Exception as e:
                    print(f"Error setting avg_price value: {str(e)}")
                    self.expenses_table.setItem(i, 10, QTableWidgetItem("Soon"))

                # Date Added - column 11
                try:
                    if 'date_added' in row and pd.notna(row['date_added']):
                        date_added = str(row['date_added']).strip()
                        self.expenses_table.setItem(i, 11, QTableWidgetItem(date_added))
                    else:
                        self.expenses_table.setItem(i, 11, QTableWidgetItem(""))
                except Exception as e:
                    print(f"Error setting date_added value: {str(e)}")
                    self.expenses_table.setItem(i, 11, QTableWidgetItem(""))

                # Date Purchased - column 12
                try:
                    if 'date_purchased' in row and pd.notna(row['date_purchased']) and str(row['date_purchased']).strip() != '' and str(row['date_purchased']).strip().lower() != 'none':
                        ...
                        self.expenses_table.setItem(i, 12, QTableWidgetItem(date_purchased))
                    else:
                        self.expenses_table.setItem(i, 12, QTableWidgetItem(""))
                except Exception as e:
            ...
            print(f"Error updating expenses list: {e}")
            # Show error in table
            self.expenses_table.setRowCount(1)
            self.expenses_table.setItem(0, 0, QTableWidgetItem(f"Error loading data: {str(e)}"))
    ...
    def mark_as_purchased(self):
        selected_rows = self.expenses_table.selectedItems()
        ...
        item_name = self.expenses_table.item(row, 0).text()
        ...
        current_price = 0.0
        try:
            current_price_text = self.expenses_table.item(row, 9).text()
        ...
        if current_price == 0.0:
            item_index = self.expenses_df[self.expenses_df['item_name'] == item_name].index
            if len(item_index) > 0:
                item_data = self.expenses_df.loc[item_index[0]]
        ...
        if len(item_index) > 0:
            ...
            self.expenses_df.loc[item_index, 'status'] = 'Purchased'
            self.expenses_df.loc[item_index, 'date_purchased'] = datetime.now().strftime('%Y-%m-%d')

            # Update current_price with actual purchase price
            self.expenses_df.loc[item_index, 'current_price'] = actual_price

            # Calculate and update avg_price
            avg_price = self.calculate_avg_price(item_name, actual_price)
            self.expenses_df.loc[item_index, 'avg_price'] = avg_price

            # Save to CSV
            self.data['expenses_list'] = self.expenses_df
            self.expenses_df.to_csv('data/expenses_list.csv', index=False)

            # Update the inventory if the item exists there
            quantity = float(self.expenses_table.item(row, 3).text())
            unit = self.expenses_table.item(row, 4).text()
            ...
            self.update_budget_tracking_after_purchase()
    ...
    def calculate_avg_price(self, item_name, new_price):
        ...
        try:
            ...
            purchased_items = self.expenses_df[
                (self.expenses_df['item_name'] == item_name) &
                (self.expenses_df['status'] == 'Purchased') &
                (self.expenses_df['current_price'].notna())
            ]
    ...
    def mark_as_cancelled(self):
        ...
        if len(item_index) > 0:
            # Update status
            self.expenses_df.loc[item_index, 'status'] = 'Cancelled'
    ...
    def edit_selected_item(self):
        """Edit the selected expenses list item"""
        selected_rows = self.expenses_table.selectedItems()
        if not selected_rows:
    ...
    def show_edit_dialog(self, item_data, item_index):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            self.expenses_df.loc[item_index, 'category'] = category_combo.currentText()
            self.expenses_df.loc[item_index, 'quantity'] = quantity_spin.value()
            self.expenses_df.loc[item_index, 'unit'] = unit_combo.currentText()
            self.expenses_df.loc[item_index, 'priority'] = priority_combo.currentText()
            self.expenses_df.loc[item_index, 'estimated_cost'] = cost_spin.value()
            self.expenses_df.loc[item_index, 'location'] = location_combo.currentText()
            self.expenses_df.loc[item_index, 'notes'] = notes_edit.text()
    ...
    def remove_item(self):
        ...
        reply = QMessageBox.question(
            self, 
            "Confirm Removal", 
            f"Are you sure you want to remove {item_name} from the expenses list?",
        ...
        if reply == QMessageBox.Yes:
            ...
            if len(item_index) > 0:
                # Remove the item
                self.expenses_df = self.expenses_df.drop(item_index)
    ...
    def setup_add_item_tab(self):
        ...
        # Add subheader
        header = QLabel("Add Item to Expenses List")
        header.setFont(QFont("Arial", 14, QFont.Bold))
        ...
        form_layout.addRow("Inventory Category:", self.new_category)

        # Budget Category - separate from inventory categories
        self.new_budget_category = QComboBox()
        self.new_budget_category.setEditable(False)
        self.new_budget_category.setMinimumContentsLength(15)
        self.new_budget_category.setSizeAdjustPolicy(QComboBox.AdjustToMinimumContentsLengthWithIcon)

        # Populate main budget categories only (Parent categories)
        budget_categories_added = False
        if 'budget_categories' in self.data and len(self.data['budget_categories']) > 0:
            budget_categories_df = self.data['budget_categories']
            if 'category_name' in budget_categories_df.columns and 'category_type' in budget_categories_df.columns:
                # Get only parent (main) categories
                main_categories = budget_categories_df[
                    budget_categories_df['category_type'] == 'Parent'
                ]['category_name'].unique()
                if len(main_categories) > 0:
                    self.new_budget_category.addItems(sorted(main_categories))
                    budget_categories_added = True

        # Default main budget categories if none exist (exact 3 categories)
        if not budget_categories_added:
            self.new_budget_category.addItems(["Kitchen Essentials", "Maintenance", "Gas"])
        form_layout.addRow("Budget Category:", self.new_budget_category)
        
        # Connect item selection to auto-fill category
        self.new_item_name.currentTextChanged.connect(self.auto_fill_item_details)

        # Connect budget category selection to update inventory categories
        self.new_budget_category.currentTextChanged.connect(self.update_inventory_categories_for_budget)
        ...
        location_values = []
        try:
            if 'location' in self.expenses_df.columns and len(self.expenses_df) > 0:
                location_values.extend([str(s) for s in self.expenses_df['location'].unique() if pd.notna(s) and str(s).strip() != ''])
            if 'store' in self.expenses_df.columns and len(self.expenses_df) > 0:
                location_values.extend([str(s) for s in self.expenses_df['store'].unique() if pd.notna(s) and str(s).strip() != ''])
        ...
        self.add_item_button = QPushButton("Add to Expenses List")
        self.add_item_button.clicked.connect(self.add_expenses_item)
    ...
    def add_expenses_item(self):
        """Add a new item to the expenses list"""
        # Get values from form
        item_name = self.new_item_name.currentText().strip()
        category = self.new_category.currentText().strip()
        budget_category = self.new_budget_category.currentText().strip()
        ...
        for item in items_to_add:
            ...
            new_id = 1
            if len(self.expenses_df) > 0 and 'item_id' in self.expenses_df.columns:
                new_id = self.expenses_df['item_id'].max() + 1 if not self.expenses_df.empty else 1
            ...
            # Add to dataframe
            self.expenses_df = pd.concat([self.expenses_df, new_item], ignore_index=True)
        ...
        # Update budget spending tracking
        budget_category = self.new_budget_category.currentText()
        total_cost = sum(float(item.get('cost', 0)) * int(item.get('quantity', 1)) for item in added_items)
        self.update_budget_spending(budget_category, total_cost)

        # Refresh budget categories dropdown
        self.refresh_budget_categories_dropdown()

        # Show success message
        if len(added_items) == 1:
            QMessageBox.information(self, "Success", f"Item '{added_items[0]}' added to expenses list.")
        else:
            QMessageBox.information(self, "Success", f"{len(added_items)} items added to expenses list: {', '.join(added_items[:5])}{' and more...' if len(added_items) > 5 else ''}")
    ...
    def sync_with_items_database(self, item_name, category, unit, cost):
        ...
        if not category_exists and category:
            ...
            new_category = pd.DataFrame({
                ...
                'description': [f"Auto-created when adding {item_name} to expenses list"]
            })
    ...
    def add_missing_ingredients(self):
        """Find missing ingredients from recipes and add them to expenses list"""
        try:
            missing_items = []
            ...
            # Get current expenses list items
            shopping_items = set()
            if len(self.expenses_df) > 0 and 'item_name' in self.expenses_df.columns:
                shopping_items = set(self.expenses_df['item_name'].str.lower())
            ...
            for _, recipe in self.data['recipes'].iterrows():
                if 'ingredients' in recipe and pd.notna(recipe['ingredients']):
                    ...
                    for ingredient in ingredients:
                        ...
                        # Check if ingredient is missing from both inventory and expenses list
            ...
            if not missing_items:
                QMessageBox.information(self, "No Missing Items", "All recipe ingredients are available in inventory or already in expenses list.")
                return
    ...
    def add_selected_missing_items(self, selected_items):
        """Add selected missing items to expenses list"""
        try:
            from datetime import datetime
            added_count = 0
            ...
            for item in selected_items:
                ...
                new_item = pd.DataFrame({
                    ...
                    'budget_category': ['Kitchen Essentials'],  # Default budget category
            ...
            QMessageBox.information(self, "Success", f"Added {added_count} missing ingredients to expenses list!")

        except Exception as e:
            print(f"Error adding missing items: {e}")
            QMessageBox.warning(self, "Error", f"Error adding missing items: {str(e)}")

    def setup_expenses_history_tab(self):
        # Create layout for the tab
        layout = QVBoxLayout(self.expenses_history_tab)
        
        # Add subheader
        header = QLabel("Expenses History")
        header.setFont(QFont("Arial", 14, QFont.Bold))
        layout.addWidget(header)
        ...
        self.history_status_combo.currentIndexChanged.connect(self.update_expenses_history)
        ...
        self.history_table.setHorizontalHeaderLabels([
            "Item", "Inventory Category", "Budget Category", "Quantity", "Unit",
    ...
    def update_expenses_history(self):
        """Update the expenses history table with completed/cancelled items"""
        # Get filter value
        status_filter = self.history_status_combo.currentText()
        
        # Get completed/cancelled items
        history_df = self.expenses_df.copy()
        ...
        for i, (_, row) in enumerate(history_df.iterrows()):
            ...
            # Set inventory category if it exists
            self.history_table.setItem(i, 1, QTableWidgetItem(row['category'] if 'category' in row else ""))

            # Set budget category if it exists
            budget_category = str(row['budget_category']) if 'budget_category' in row and pd.notna(row['budget_category']) else "Kitchen Essentials"
            self.history_table.setItem(i, 2, QTableWidgetItem(budget_category))

            # Set quantity
            self.history_table.setItem(i, 3, QTableWidgetItem(str(row['quantity']) if 'quantity' in row else ""))

            # Set unit if it exists
            self.history_table.setItem(i, 4, QTableWidgetItem(row['unit'] if 'unit' in row else ""))
            
            # Last Price - column 5
            if 'last_price' in row and pd.notna(row['last_price']):
                self.history_table.setItem(i, 5, QTableWidgetItem(f"{currency_symbol} {float(row['last_price']):.2f}"))
            elif 'estimated_cost' in row and pd.notna(row['estimated_cost']):
                self.history_table.setItem(i, 5, QTableWidgetItem(f"{currency_symbol} {float(row['estimated_cost']):.2f}"))
            else:
                self.history_table.setItem(i, 5, QTableWidgetItem("Soon"))

            # Current Price - column 6
            if 'current_price' in row and pd.notna(row['current_price']):
                self.history_table.setItem(i, 6, QTableWidgetItem(f"{currency_symbol} {float(row['current_price']):.2f}"))
            else:
                self.history_table.setItem(i, 6, QTableWidgetItem("Soon"))

            # Avg Price - column 7
            if 'avg_price' in row and pd.notna(row['avg_price']):
                self.history_table.setItem(i, 7, QTableWidgetItem(f"{currency_symbol} {float(row['avg_price']):.2f}"))
            else:
                self.history_table.setItem(i, 7, QTableWidgetItem("Soon"))
            ...
            try:
                ...
                if 'location' in row and pd.notna(row['location']) and str(row['location']).strip() != '':
                    ...
                    location_value = str(row['store']).strip()
                self.history_table.setItem(i, 8, QTableWidgetItem(location_value))
            except Exception as e:
            ...
            try:
                if 'status' in row and pd.notna(row['status']) and row['status'] != '':
                    ...
                    self.history_table.setItem(i, 9, QTableWidgetItem(status_value))
                else:
            ...
            # Date Purchased - column 10
            if 'date_purchased' in row and pd.notna(row['date_purchased']) and str(row['date_purchased']).strip() != '':
                self.history_table.setItem(i, 10, QTableWidgetItem(str(row['date_purchased'])))
            else:
    ...
    def update_inventory_quantity(self, item_name, quantity, unit):
        ...
        # Get the current price from expenses list
        shopping_item = self.expenses_df[self.expenses_df['item_name'] == item_name]
        if len(shopping_item) == 0:
            print(f"ERROR: Could not find {item_name} in expenses list")
            return
        ...
        if len(matching_inventory) > 0:
            ...
            new_inventory_item = {
                ...
                'description': f"Added from expenses list purchase",
    ...
    def refresh_all_tabs_after_purchase(self):
        ...
        try:
            ...
            if hasattr(self, 'main_app') and self.main_app and hasattr(self.main_app, 'load_data'):
                ...
                # Refresh the current expenses list display
    ...
    def load_expenses_data(self):
        """Reload expenses data from the main data source"""
        try:
            if 'expenses_list' in self.data:
                self.expenses_df = self.data['expenses_list'].copy()
                print("DEBUG: Expenses data reloaded successfully")
            else:
                print("DEBUG: No expenses_list found in data")
        except Exception as e:
            print(f"Error loading expenses data: {e}")

    def open_bulk_pricing_editor(self):
        """Open bulk pricing editor for expenses list items"""
        try:
            ...
            # Show as dialog
            dialog = QDialog(self)
            dialog.setWindowTitle("Bulk Pricing Editor - Expenses List")
            dialog.setModal(True)
    ...
    def on_bulk_pricing_updated(self, updated_items):
        ...
        try:
            # Update expenses list with new pricing
            for item in updated_items:
                item_name = item['name']
                new_price = item['new_price']

                # Update in expenses dataframe
                mask = self.expenses_df['item_name'] == item_name
                if mask.any():
                    self.expenses_df.loc[mask, 'current_price'] = new_price
                    self.expenses_df.loc[mask, 'last_price'] = new_price
            ...
            QMessageBox.information(self, "Success", f"Updated pricing for {len(updated_items)} items in expenses list.")
    ...
    def refresh_data(self):
        """Refresh expenses list data from CSV file"""
        try:
            # Reload expenses list from CSV
            expenses_file = 'data/expenses_list.csv'
            if os.path.exists(expenses_file):
                self.expenses_df = pd.read_csv(expenses_file)
                self.data['expenses_list'] = self.expenses_df
                ...
                QMessageBox.information(self, "Refresh Complete",
                    f"Shopping list data refreshed successfully!\n\nLoaded {len(self.expenses_df)} items from CSV file.")

                print(f"[SUCCESS] Shopping list refreshed: {len(self.expenses_df)} items loaded")

            else:
                QMessageBox.warning(self, "File Not Found",
                    f"Expenses list CSV file not found at: {expenses_file}")

        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Error refreshing expenses data: {e}")
            QMessageBox.critical(self, "Refresh Error",
                f"Failed to refresh expenses list data:\n{str(e)}")
            print(f"[ERROR] Error refreshing expenses data: {e}")

    def refresh_budget_categories_dropdown(self):
        """Refresh the budget categories dropdown with latest data"""
        try:
            # Clear current items
            ...
            print(f"Error refreshing budget categories dropdown: {e}")
            # Fallback to default categories
            self.new_budget_category.clear()
            self.new_budget_category.addItems(["Kitchen Essentials", "Utilities", "Maintenance", "Equipment", "Other"])

    def update_inventory_categories_for_budget(self, budget_category):
        """Update inventory categories dropdown based on selected budget category"""
        try:
            # Clear current inventory categories
            current_category = self.new_category.currentText()
            self.new_category.clear()

            # Get sub-categories based on the exact budget category structure
            available_categories = []

            if budget_category == "Kitchen Essentials":
                # Show ALL inventory categories for Kitchen Essentials
                if 'categories' in self.data and len(self.data['categories']) > 0:
                    categories_df = self.data['categories']
                    available_categories = categories_df['category_name'].dropna().unique().tolist()
                else:
                    # Fallback to predefined inventory categories
                    available_categories = [
                        'Grains', 'Baking', 'Spices', 'Oils', 'Meat', 'Poultry',
                        'Vegetables', 'Dairy', 'Fruits', 'Legumes', 'Prepared Foods',
                        'Beverages', 'Nuts & Seeds', 'Condiments', 'Seafood', 'Eggs',
                        'Bread & Bakery', 'Sweeteners', 'Aromatics'
                    ]

            elif budget_category == "Maintenance":
                # Show only Maintenance sub-categories: Repairing, Spares
                available_categories = ["Repairing", "Spares"]

            elif budget_category == "Gas":
                # Show only Gas sub-categories: Indian Gas, HP Gas, Local Gas Supply
                available_categories = ["Indian Gas", "HP Gas", "Local Gas Supply"]

            else:
                # For any other budget categories, show generic options
                available_categories = ["Other", "Miscellaneous"]
            ...
            if available_categories:
                self.new_category.addItems(available_categories)

                # Try to restore previous selection if it's still available
                if current_category in available_categories:
                    self.new_category.setCurrentText(current_category)
            else:
                self.new_category.addItems(["Uncategorized"])

        except Exception as e:
            print(f"Error updating inventory categories for budget: {e}")
            # Fallback to basic categories
            self.new_category.clear()
            self.new_category.addItems(["Uncategorized", "Other"])

    def update_budget_spending(self, budget_category, amount):
        """Update budget spending when expenses are added"""
        try:
            # Try to get budget manager instance to update spending
            budget_manager = getattr(self, 'budget_manager', None)
            if budget_manager and hasattr(budget_manager, 'update_budget_spending'):
                success = budget_manager.update_budget_spending(budget_category, amount)
                if success:
                    print(f"Updated budget spending for '{budget_category}': +₹{amount:,.2f}")
                else:
                    print(f"Failed to update budget spending for '{budget_category}'")
            else:
                print("Budget manager not available for spending tracking")

        except Exception as e:
            print(f"Error updating budget spending: {e}")

    def update_budget_tracking_after_purchase(self):
        """Update budget tracking after an expense is marked as purchased"""
        try:
            # Check if main app has budget manager
            if hasattr(self, 'main_app') and self.main_app:
                # Try to find budget manager widget and refresh it
                # This will update the budget tracking with the latest expense data
                if hasattr(self.main_app, 'data'):
                    # Update the main app's data with our latest expenses data
                    self.main_app.data['expenses_list'] = self.expenses_df

                    # If there's a budget manager instance, refresh it
                    # Note: This would need to be implemented when the budget manager is instantiated
                    print("✅ Budget tracking data updated after purchase")

        except Exception as e:
            print(f"Error updating budget tracking after purchase: {e}")
